<div class="row">
                ...
                <div class="tab-pane fade @if($loop->first) active in show @endif" id="{{ $key }}-tab-content"
                    ...
                    <div class="form-group">
                        ...
                        <textarea type="text" name="description[{{ $key }}]" id="description-{{ $key }}" cols="30"
                                  ...
                                  rows="15"
                        >{{ isset($aboutUsContent) ? $aboutUsContent->descriptionTranslate?->{$key} : (old('description.'.$key) ?? '') }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea type="text" name="content[{{ $key }}]" id="content-{{ $key }}" cols="30"
                                  ...
                                  rows="15"
                        >{{ isset($aboutUsContent) ? $aboutUsContent->contentTranslate?->{$key} : (old('content.'.$key) ?? '') }}</textarea>
                        @error('content.' . $key)
                        <span class="error invalid-feedback">{{ $message }} </span>
                        @enderror
                    </div>
    ...
    <div class="col-md-12">
        <div class="form-group required">
            ...
            <span class="error invalid-feedback"> {{ $message }} </span>
            @enderror
            @if(isset($aboutUsContent) && $aboutUsContent->image_url != null)
                <img id="image-preview" class="rounded about-us-image-edit" src="{{ $aboutUsContent->image_url }}"
                     alt="">
                <button type="button" class="btn btn-sm btn-danger"
                        id="deleteImage"
                        data-id="{{ $aboutUsContent->id }}">

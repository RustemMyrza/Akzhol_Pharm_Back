            ...
            <div class="row">
                ...
                <div class="col-12">
                    <div class="card-tools mb-2 mb-md-3">
                        ...
                        <a href="{{ route('admin.filterItems.create', ['filter' => $filter]) }}"
                    ...
                    <div class="info-box info-card flex-column shadow-none">
                        <div class="table-responsive" id="for_sort">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                @forelse($filterItems as $filterItem)
                                    <tr>
                                        <td data-original-id="{{ $filterItem->id }}"
                                            data-main-id="{{ $filter->id }}"
                                            data-id="{{ $filterItem->id }}">
                                            {{ $filterItem->id }}
                                        </td>
                                        <td>{{ $filterItem->titleTranslate?->ru }}</td>
                                        <td>
                                            <label class="checkbox-label">
                                                <input id="checkbox" class="checkbox cb cb1 update_is_active"
                                                       ...
                                                       data-id="{{ $filterItem->id }}"
                                                       data-main-id="{{ $filter->id }}"
                                                    {{ $filterItem->is_active == 1 ? 'checked' : '' }} />
                                                <i></i>
                                            </label>
                                        </td>
                                        <td>
                                            <div class="card-tools">
                                                <a href="{{ route('admin.filterItems.edit', ['filterItem' => $filterItem, 'filter' => $filter]) }}"
                                                   title="@lang('messages.edit')"
                                                   class="btn btn-primary btn-sm">
                                                    @lang('messages.edit')
                                                </a>
                                                <form method="POST" class="d-inline"
                                                      action="{{ route('admin.filterItems.destroy', ['filterItem' => $filterItem, 'filter' => $filter]) }}">
                                                    @method('DELETE')
                                                    @csrf
                            ...
                            <div class="pagination-wrapper">
                                {!! $filterItems->appends(['search' => request('search')])->render() !!}
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $('.update_is_active').click(function (e) {
            ...
            if ($(this).prop('checked')) {
                $.ajax({
                    method: "POST",
                    url: `/admin/filters/${dataMainId}/filterItems/${dataId}/updateIsActive`,
                    data: {
                        _token: _token,
                        is_active: 1,
                        data_id: dataId,

                ...
                <div class="tab-pane fade @if($loop->first) active in show @endif" id="{{ $key }}-tab-content"
                    ...
                    <div class="form-group required ">
                        ...
                        <input class="form-control @error('title.' . $key) is-invalid @enderror"
                               name="title[{{ $key }}]"
                               @if($feature->type == \App\Enum\FeatureTypeEnum::SELECTABLE)
                                   type="text"
                               @else
                                   type="number"
                               @endif
                               id="title-{{ $key }}"
                               value="{{ isset($featureItem) ? $featureItem->titleTranslate?->{$key} : (old('title.'.$key) ?? '') }}">
                        @error('title.' . $key)
    ...
    <div class="col-md-12">
        <div class="form-group">
            ...
            <input type="number" class="form-control @error('position') is-invalid @enderror" id="position"
                   name="position"
                   value="{{ isset($featureItem) ? $featureItem->position : (old('position') ?? $lastPosition) }}">
            @error('position')
            <span class="error invalid-feedback"> {{ $message }} </span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <label class="checkbox-label">
                <input id="checkbox" class="checkbox cb cb1" type="checkbox" name="is_active" value="1"
                    {{ isset($featureItem) ? ($featureItem->is_active == 1 ? 'checked' : '') : 'checked' }} />
                <i></i>
                <span>Активный</span>
            </label>
        </div>
